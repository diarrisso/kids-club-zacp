...
- **`Appointment`** (`app/Models/Tenant/Appointment.php`): UUID PK; fields incl. `parent_email`, `parent_first_name`, `patient_first_name`, `starts_at`/`ends_at` (cast `datetime`), `status`, `cancellation_token`; `service()` and `practitioner()` belongsTo. `Practitioner::fullName()` exists. `Service` has `name`.
