...
<template>
  <div>
    <!-- Heading -->
    <h2 class="text-[1.35rem] font-bold tracking-tight text-[#211F66]">Bestätigen</h2>
    <p class="mt-1 text-sm text-slate-400">Alles korrekt? Dann bestätige deinen Termin.</p>

    <!-- Summary card -->
    <div class="mt-4 rounded-2xl ring-1 ring-[#EC0A8C]/15 overflow-hidden"
      ...
      <dl>
        ...
        <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100/80">
          <dt class="flex items-center gap-2.5 text-xs font-medium text-slate-400 shrink-0">
            <span class="inline-flex h-6 w-6 items-center justify-center rounded-lg shrink-0"
              ...
              <svg class="h-3.5 w-3.5 text-[#EC0A8C]" viewBox="0 0 16 16" fill="currentColor">
          ...
          <dd class="text-sm font-semibold text-[#4B4A9E] text-right max-w-[58%] leading-snug">{{ serviceLabel }}</dd>
        </div>
        <!-- Datum & Zeit -->
        <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100/80">
          <dt class="flex items-center gap-2.5 text-xs font-medium text-slate-400 shrink-0">
          ...
          <dd class="text-sm font-semibold text-[#4B4A9E] text-right max-w-[60%] leading-snug">{{ formattedDate }}</dd>
        </div>
        <!-- Behandler -->
        <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100/80">
          <dt class="flex items-center gap-2.5 text-xs font-medium text-slate-400 shrink-0">
          ...
          <dd class="text-sm font-semibold text-[#4B4A9E] text-right max-w-[60%] leading-snug">{{ formattedPractitioner }}</dd>
        </div>
        <!-- Kind -->
        <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100/80">
          <dt class="flex items-center gap-2.5 text-xs font-medium text-slate-400 shrink-0">
          ...
          <dd class="text-sm font-semibold text-[#4B4A9E] text-right max-w-[60%] leading-snug">{{ patientName }}</dd>
        </div>
        <!-- E-Mail -->
        <div class="flex items-center justify-between px-4 py-3">
          ...
          <dd class="text-sm font-semibold text-[#4B4A9E] break-all text-right max-w-[62%] leading-snug">{{ formData.parent_email }}</dd>
        </div>
      </dl>
    </div>

    <!-- E-Mail info banner -->
    <div class="mt-3 flex items-center gap-2.5 rounded-xl px-3.5 py-2.5 ring-1 ring-[#EC0A8C]/20"
         style="background-color: rgba(90,122,145,0.06);">
      <svg class="h-4 w-4 shrink-0 text-[#EC0A8C]" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z" clip-rule="evenodd"/>
      </svg>
      <p class="text-xs leading-relaxed text-[#C40C78]">
        Bestätigungsmail wird an <strong>{{ formData.parent_email }}</strong> gesendet.
      </p>
    </div>

    <!-- GDPR consent -->
    <label class="mt-3 flex cursor-pointer items-start gap-3 rounded-2xl p-3.5 ring-1 ring-slate-100 transition-all duration-150 hover:ring-[#EC0A8C]/30"
      ...
      <input
        ...
        v-model="consent"
        type="checkbox"
        class="mt-0.5 h-4 w-4 shrink-0 rounded border-slate-300 focus:ring-[#FBB9C4]/50"
        :style="consent ? { accentColor: '#EC0A8C' } : {}"
      >
      <span class="text-xs leading-relaxed text-[#5A5996]">
